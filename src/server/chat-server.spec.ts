...
import * as ServerDatabase from '../database/server_database.js';
...
describe('chat-server.ts', () => {
  ...
  const client1 = new MockWebSocket('URL');
  const client2 = new MockWebSocket('URL');
  beforeEach(() => {
    user1 = new User('user1', 'password1', '@93423');
    user1.setWebsocket(client1);
    user2 = new User('user2', 'password2', '@34324');
    user1.setWebsocket(client2);

    channel1 = new DirectMessageChannel(
      'channelName',
      user1.getUUID(),
      user2.getUUID(),
      '#' + user1.getUUID() + user2.getUUID()
    );
    channel2 = new PublicChannel('channelName', '#' + 'lescode');
    chatServer = new ChatServer(
      server,
      new Set<string>(['@34324', '@93423']),
      new Set<string>(['#' + user1.getUUID() + user2.getUUID(), '#' + 'lescode'])
    );
  });
  describe('Friends', () => {
    ...
    describe('getUserByWebsocket', () => {
      ...
      it('should return the user with the given valid websocket', async () => {
        const client1 = user1.getWebSocket();
        if (client1 !== undefined) {
          const result = await chatServer.getUserByWebsocket(client1);
          expect(result).toEqual(user1);
        }
      });
    ...
    describe('cachUser', () => {
      ...
      it('should map the user websocket to the user id', async () => {
        chatServer.cachUser(user1);
        const ws = user1.getWebSocket();
        // expect(chatServer.getUserByWebsocket(client1)).toBeUndefined();
        // FIXME: bij before each ook altijd nieuwe websocket, want hier creeer je nieuwe user1 maar geen nieuwe websocket, waardoor het de webscoket niet herkent.
        if (ws !== undefined) {
          expect(await chatServer.getUserByWebsocket(ws)).toEqual(user1);
        } else {
          // fail test
        }
      });
    ...
    describe('unCacheUser', () => {
      it('should remove the user from the cached users', async () => {
        chatServer.cachUser(user1);
        const spy = vi.spyOn(UserDatabase, 'userSave').mockImplementation(() => {
  ...
  describe('Channels', () => {
    ...
    describe('getPublicChannelByChannelId', () => {
      ...
      it('should return the channel with the given valid channel id', async () => {
        ...
        const result = await chatServer.getPublicChannelByChannelId(channel2.getCUID());

        expect(result).toEqual(channel2);
      });

      it('should throw an InvalidChannelIdError when given an invalid channel id', async () => {
        expect(await chatServer.getPublicChannelByChannelId(invalidChannelId)).toEqual(undefined);
      });
      ...
      it('should throw a ChannelNotFoundError when channelLoad returns undefined', async () => {
        const spyChannelLoad = vi
          .spyOn(ChannelDatabase, 'publicChannelLoad')
          .mockReturnValue(Promise.resolve(undefined));
        expect(await chatServer.getPublicChannelByChannelId(channel2.getCUID())).toEqual(undefined);
      });
