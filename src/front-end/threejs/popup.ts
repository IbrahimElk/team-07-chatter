...
export function showPopup(building: THREE.Object3D<THREE.Event>) {
  ...
  if (buildingName === BuildingNames.nameacco){
  ...
  } else if (buildingName === ClientUser.getCurrentClassRoom(document)?.building) {
    (document.querySelector('.text') as HTMLElement).append(
    ...
    (document.querySelector('.text') as HTMLElement).append(
      'This is building ' + buildingName + ', there are no lessons given in this building at the moment' + ' If you click on this building, you will open the corresponding chat window.'
    );
