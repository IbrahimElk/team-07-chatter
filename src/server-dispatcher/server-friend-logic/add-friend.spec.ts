...
describe('addFriend', () => {
  ...
  let spygetUUIDByName: SpyInstance<[name: string], Promise<UUID | undefined>>;
  ...
  it("addFriend fails to add a friend to a user's friend list 1", async () => {
    spygetUUIDByName = vi.spyOn(chatServer, 'getUUIDByName').mockReturnValue(Promise.resolve(undefined));
    ...
    expect(spygetUUIDByName).toHaveBeenCalled();
  ...
  it("addFriend fails to add a friend to a user's friend list 2", async () => {
    spygetUUIDByName = vi.spyOn(chatServer, 'getUUIDByName').mockReturnValue(Promise.resolve(userBen.getName()));
    spygetUserByWebsocket = vi.spyOn(chatServer, 'getUserBySessionID').mockReturnValueOnce(Promise.resolve(undefined));
  ...
  it("addFriend fails to add a friend to a user's friend list 3", async () => {
    spygetUUIDByName = vi.spyOn(chatServer, 'getUUIDByName').mockReturnValue(Promise.resolve(userJan.getName()));
