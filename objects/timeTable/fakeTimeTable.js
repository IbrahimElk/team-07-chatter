 ...
 */
export function createFakeTimetable() {
    const timeTable = {
        timeSlots: [
            {
                longDescription: '',
    ...
    while (dayOfWeek <= 7) {
        ...
        while (timeofDay <= 23) {
            ...
            timeTable.timeSlots.push({
                longDescription: hashDescriptionToCourse(description),
...
function formattedTime(hours) {
    let hoursFormat = '';
    if (hours < 10)
        hoursFormat = '0' + hours.toString();
    else
        hoursFormat = hours.toString();
 ...
 */
function hashDescriptionToBuilding(description) {
    const buildings = [
        '200 K',
        '200 S',
        '200 M',
 ...
 * @returns A Building name.
 */
function hashDescriptionToCourse(description) {
    const courses = [
...
//# sourceMappingURL=fakeTimeTable.js.map