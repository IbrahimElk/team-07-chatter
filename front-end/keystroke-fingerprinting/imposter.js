 ...
 * @author thomasevenepoel
 */
export function Detective(map_in_database, map_sent_by_user, maps_of_other_users) {
 ...
 */
export function calculateDelta(timings, n) {
    ...
    for (let i = n - 1; i < timings.length; i++) {
        let substring = '';
        let j = i - n + 1;
        let max = timings.at(j)?.[1];
        let min = timings.at(j)?.[1];
        while (j < i + 1) {
            const element = timings.at(j);
            substring += element[0];
            if (element[1] < min) {
                min = element[1];
            }
 ...
 * @author thomasevenepoel
 */
export function CompareTwoMaps(first_map, second_map) {
    ...
    for (const substring of keys_first_map_common_elements) {
        const difference = Math.abs(keys_first_map_common_elements.indexOf(substring) - keys_second_map_common_elements.indexOf(substring));
 ...
 * @author thomasevenepoel
 */
export function orderDelta(delta_timings) {
 ...
 * @author thomasevenepoel
 */
export function rMeasure(ordering_list) {
 ...
 * @author thomasevenepoel
 */
export function aMeasure(map_sent_by_user, map_in_database) {
    const t = 1.4;
    const map1_common_keys = new Map();
    const map2_common_keys = new Map();
    for (const substring of map_sent_by_user.keys()) {
        if (map_in_database.has(substring)) {
            map1_common_keys.set(substring, map_sent_by_user.get(substring));
        }
    }
    for (const substring of map_in_database.keys()) {
        if (map_sent_by_user.has(substring)) {
            map2_common_keys.set(substring, map_in_database.get(substring));
        }
    }
