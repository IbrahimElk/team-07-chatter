...
export function makeTree(posX, posZ, kindOfTree, size) {
    const group = generateTree(kindOfTree, size);
    group.position.x = posX;
    group.position.z = posZ;
    scene.add(group);
}
function generateTree(kindOfTree, size) {
...
export function makeMaterial(mycolor) {
    const mat = new THREE.MeshStandardMaterial({ color: mycolor });
    return mat;
}
// functions to create buildings:
export function makeGeo(xlength, height, zlength, xpos, zpos) {
    const geo = new THREE.BoxGeometry(xlength, height, zlength);
    geo.translate(xpos, Heights.heightsaver + height * 0.5, zpos);
    return geo;
}
export function makeBench(xpos, zpos, rotation) {
...
export function finishingTouches(building, name, layer, castShadowB) {
...
export function makePath(xlength, zlength, xpos, zpos, ydregree) {
...
function makePathGeo(xlength, zlength, xpos, zpos, ydregree) {
...
export function makeParking(xlength, zlength, xpos, zpos, ydregree) {
    const parking = new THREE.Mesh(makePathGeo(xlength, zlength, xpos, zpos, ydregree), makeMaterial(0xc7c1c1));
    parking.receiveShadow = true;
    scene.add(parking);
}
//# sourceMappingURL=functionsForLayout.js.map