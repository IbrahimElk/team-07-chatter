 ...
 */
export function channelDelete(channel) {
    let path = '';
    if (channel instanceof PublicChannel)
        path = './assets/database/public-channels/';
    if (channel instanceof DirectMessageChannel)
        path = './assets/database/direct-message-channels/';
 ...
 */
export async function channelSave(channel) {
    let path = '';
    if (channel instanceof PublicChannel)
        path = './assets/database/public-channels/';
    if (channel instanceof DirectMessageChannel)
    ...
    try {
        ...
        fs.writeFileSync(path, arrayBufferToString(encryptedChannel.iv) + '\n' + arrayBufferToString(encryptedChannel.encryptedObject));
    }
    catch (error) {
 ...
 * @returns
 */
export async function publicChannelLoad(identifier) {
    ...
    if (savedChannelCheck !== undefined) {
        ...
        for (const message of savedChannelCheck.messages) {
            savedChannel.addMessage(Object.assign(tempChannel, new Message(new User('temp', 'user'), message.DATE, message.TEXT, 0), message));
        }
 ...
 * @returns
 */
export async function friendChannelLoad(identifier) {
    const savedChannelCheck = await loadingChannel(identifier, './assets/database/direct-message-channels/');
    if (savedChannelCheck !== undefined) {
        const tempChannel = new DirectMessageChannel(new User('fakeUser', 'fakePassword'), new User('fakeUser', 'fakePassword'));
        const savedChannel = Object.assign(tempChannel, savedChannelCheck);
        for (const message of savedChannelCheck.messages) {
            savedChannel.addMessage(Object.assign(tempChannel, new Message(new User('temp', 'user'), message.DATE, message.TEXT, 0), message));
        }
 ...
 * @returns
 */
async function loadingChannel(identifier, path) {
...
//# sourceMappingURL=channel_database.js.map